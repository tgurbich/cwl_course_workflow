...
def main(mash, genomes_file, outfolder, infolder):
    scores = dict()
    genomes_to_evaluate = load_list(genomes_file)
    with open(mash, 'r') as infile:
        for line in infile:
            ...
            if genome in genomes_to_evaluate:
                ...
                if genome in scores:
                    ...
                    scores[genome] = score
    discard, new_strains, new_species = evaluate(genomes_to_evaluate, scores)
...
def evaluate(genomes_to_evaluate, scores):
    discard = set()
    new_strains = set()
    new_species = set()
    for genome in genomes_to_evaluate:
...
def parse_args():
    parser = argparse.ArgumentParser(description='Processes the mash dist file, saves new strains and new species '
                                                 'into new folders and reports a list of genomes to discard')
